...
input_dir = "/Users/jessamyn/Pictures/01_Gallery"
max_dimension = 1920

admin_url = "http://127.0.0.1:8000/django-admin"
admin_username = 'admin2'

# admin_url = "https://roxy-the-renovator.herokuapp.com/django-admin"
# admin_username = 'admin2'
...
def create_gallery(driver, gallery_name, gallery_exif, photo_names):
    driver.get("%s/photologue/gallery/add/" % admin_url)
    fields = {
        'title': gallery_name,
...
def create_photo(driver, photo_file_name, image_exif, img):
    driver.get("%s/photologue/photo/add/" % admin_url)
            ...
            try:
                with open(str(os.path.join(root, file)), 'rb') as f:
                    ...
                    if extension.lower() == '.png':
                        img = img.convert('RGB')
                        jpg_file = str(os.path.join(root, '{}.jpg'.format(filename)))
                        img.save(jpg_file)
                        f = open(jpg_file, 'rb')
                        img = Image.open(f)
